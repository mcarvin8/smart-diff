...
function mockLlmClient(content: string): () => Promise<import('../src/ai/openAIConfig').OpenAiLikeClient> {
  return async () =>
    ({
      chat: {
        completions: {
...
describe('generateSummary', () => {
  const commits: CommitInfo[] = [{ hash: 'deadbeef', message: 'feat: example' }];
  ...
  it('throws when LLM gateway is off and no client provider', async () => {
    ...
    await expect(generateSummary('+added line', ['src/a.ts'], commits, flagsBase)).rejects.toThrow(
      LLM_GATEWAY_REQUIRED_MESSAGE
    );
  });
    ...
    expect(completionCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        messages: expect.arrayContaining([
          expect.objectContaining({ role: 'system', content: 'You are a test bot.' }),
          expect.objectContaining({ role: 'user', content: expect.stringContaining('Team: QA') }),
        ]),
      })
    );
  });
  ...
  it('defaults model to gpt-4o-mini when omitted', async () => {
    ...
    expect(completionCreate).toHaveBeenCalledWith(expect.objectContaining({ model: 'gpt-4o-mini' }));
  });
  ...
  it('includes exclude-only commit filter copy in user message', async () => {
    ...
    const userMsg = completionCreate.mock.calls[0]![0].messages.find((m: { role: string }) => m.role === 'user')?.content as string;
  ...
  it('embeds diffSummary JSON when provided', async () => {
    ...
    const diffSummary = { files: [], totalFiles: 0, totalAdditions: 0, totalDeletions: 0 };
  ...
  it('uses 4000 max_tokens when OPENAI_MAX_TOKENS is invalid', async () => {
    ...
    await generateSummary('d', [], [], flagsBase);
    expect(completionCreate).toHaveBeenCalledWith(expect.objectContaining({ max_tokens: 4000 }));
