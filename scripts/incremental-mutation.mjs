...
// with the base branch. Skip mutation testing when nothing changed.
...
function main() {
  ...
  if (files.length === 0) {
    console.log(
      "[incremental-mutation] No source files changed; skipping mutation testing.",
    );
    return;
  }

  console.log(
    `[incremental-mutation] Running Stryker against ${files.length} changed file(s):`,
  );
