...
describe('getCommits', () => {
  it('returns log.all as CommitInfo[]', async () => {
    ...
    await expect(getCommits(git, 'from', 'to')).resolves.toEqual([{ hash: 'aaa', message: 'msg' }]);
...
describe('getDiff', () => {
  it('uses range diff and repoRootOverride skips revparse', async () => {
    ...
    const out = await getDiff(git, 'a', 'b', commits, false, { excludeFolders: ['out'] }, join(__dirname, 'fixture-repo'));

    expect(out).toBe('range-diff');
    expect(diff).toHaveBeenCalledWith(['a..b', '--', '.', ':(exclude)out']);
    expect((git as unknown as { revparse: jest.Mock }).revparse).not.toHaveBeenCalled();
  });
  ...
  it('joins per-commit patches and drops empty', async () => {
    ...
    const out = await getDiff(git, 'f', 't', commits, true, undefined, join(__dirname, 'fixture-repo'));
  ...
  it('aggregates range numstat and name-status', async () => {
    ...
    diff.mockImplementation(async (args: string[]) => {
      if (args.includes('--numstat')) {
        return ['1\t2\tadded.ts', '-\t-\tempty', '3\t0\tnew/name', '1\t1\tdup.ts', '2\t0\tprefix{a => b}'].join('\n');
      }
    ...
    expect(summary.files.find((f) => f.path === 'new/name')).toMatchObject({ status: 'renamed' });
    expect(summary.files.find((f) => f.path === 'prefixb')).toBeDefined();
    expect(summary.files.find((f) => f.path === 'unknown.bin')?.status).toBe('unknown');
  });
